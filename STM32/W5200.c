...
extern unsigned int  sch_packet_UDP_reciv;
...
void loopback_tcps(SOCKET s, uint16 port)
{
	...
	case SOCK_ESTABLISHED:					// if connection is established 
		if(ch_status[s]==1)
		{
			...
			Transf(": Connected");
                        Transf("\r\n Peer IP : ");
                        itoa(IINCHIP_READ(Sn_DIPR0(s)+0),strng1);
                        Transf(strng1);
                        Transf(".");
                        itoa(IINCHIP_READ(Sn_DIPR0(s)+1),strng1);
                        Transf(strng1);
                        Transf(".");
                        itoa(IINCHIP_READ(Sn_DIPR0(s)+2),strng1); 
                        Transf(strng1);
                        Transf(".");
                        itoa(IINCHIP_READ(Sn_DIPR0(s)+3),strng1);
                        Transf(strng1);
                        Transf("\r\n");
                        Transf("Peer Port : " );
                        itoa(( (uint16)(IINCHIP_READ(Sn_DPORT0(s)+0)<<8) +(uint16)IINCHIP_READ(Sn_DPORT0(s)+1)),strng1);
                        Transf(strng1);
                        Transf("."); 	
		...
		{
			if (RSR_len > buf_tcp) RSR_len = buf_tcp;	// if Rx data size is lager than TX_RX_MAX_BUF_SIZE 
...
//-----------
            ...
            sent_data_len = send(s, buf, received_len, (bool)WINDOWFULL_FLAG_OFF);         // sent the received data 
	...
	case SOCK_CLOSE_WAIT:                           	// If the client request to close 
		...
		Transf("\r\n");
                if ((RSR_len = getSn_RX_RSR(s)) > 0) 		// check Rx data 
		{
			if (RSR_len > buf_tcp) RSR_len = buf_tcp;	// if Rx data size is lager than buf_tcp 
																												// the data size to read is MAX_BUF_SIZE. 
			received_len = recv(s, data_buf, RSR_len);		// read the received data 
		}
		disconnect(s);
	...
	case SOCK_CLOSED:                                       // if a socket is closed 
		if(!ch_status[s]) 
		{
			...
			itoa(s,strng1);
            Transf(strng1);
			Transf(": Loop-Back TCP Server Started. port :");
			itoa(port,strng1);
            Transf(strng1);
			Transf("\r\n");
...
{							
	...
	case SOCK_ESTABLISHED:						// if connection is established                
		...
		if ((RSR_len = getSn_RX_RSR(s)) > 0) 			// check Rx data 
		{
			...
			received_len = recv(s, data_buf, RSR_len);			// read the received data 
			//IO(data_buf,lsr);  // UDP
			data_buf=text_buffer;
    		received_len=text_lengh_TCP;
    		text_lengh_TCP=0;
                        ...
                        if(sent_data_len != received_len) // ohly assert when windowfull 
                        {
                                  ...
                                  {
                                            Transf("ret :");
                                            snprintf(strng1,"%d",sent_data_len);
                							Transf(strng1);
											Transf("\r\n");
...
Transf("\r\n");
	snprintf(strng1,"%d",s);
                   Transf(strng1);                        		// If the client request to close 
		Transf(": CLOSE_WAIT");
		Transf("\r\n");
	...
	case SOCK_CLOSED:                                               // if a socket is closed               
		if(!ch_status[s])
		{
			...
			snprintf(strng1,"%d",s);
            Transf(strng1);
			Transf(": Loop-Back TCP Client Started. port: ");
			snprintf(strng1,"%d",port);
            Transf(strng1);
            Transf("\r\n");
		...
		if(socket(s, Sn_MR_TCP, port, 0x00) == 0)    // reinitialize the socket 
		{
			...
			snprintf(strng1,"%d",s);
            Transf(strng1);
			Transf("\a : Fail to create socket.");
			Transf("\r\n");
...
void loopback_udp(SOCKET s, uint16 port,unsigned char event)
{
	 int16 RSR_len;
    uint16 received_len;
	uint32 lsr=0;
	...
	case SOCK_UDP:
				...
				if ((FLAG_UDP_EVENT>0u)&&(FLAG_UDP_INSTALL==1u))
					{
						received_len=16+16+104; //длинна ответной квитанции

						sch_tx_packet++;//счётчик отправленных пакетов
						
						if(sendto(s, TX_BUF, received_len,(uint8*)&destip_UDP, destport_UDP) == 0)	 // send the message data 
										{
	...
	Transf("\r\n");
	itoa(s,strng1);
    Transf(strng1);                                             /* if a socket is closed */
...
for (i=0;i<TX_RX_MAX_BUF_SIZE;++i) data_buf[i]=0;

 switch (getSn_SR(s))	
 {//   Get the state of socket s 
    case SOCK_ESTABLISHED :   // If the socket is established 	    
      	if(ch_status[s] == ready_state) 
      							{u_out("\r\nTELNET server established via SOCKET ",s);
	      ...
	      {             
	        if (RSR_len > TX_RX_MAX_BUF_SIZE) RSR_len = TX_RX_MAX_BUF_SIZE;	// if Rx data size is lager than TX_RX_MAX_BUF_SIZE 
				...
				u_out("received_len:",received_len);
				Transf(data_buf);
				IO(data_buf);  //  TCP/IP
				//received_len=text_lengh_TCP;
				received_len=5;
	            ...
	            sent_data_len = send(s, text_buffer_TCP, received_len, (bool)WINDOWFULL_FLAG_OFF);         // sent the received data    
	      }
      break;    
...
void init_IP_adress (unsigned char slave,unsigned char adress)
{
	unsigned char adr=0;

	if (slave==0) adr = 1;  else adr = 4;//основная или резервная подсетка
/*
	switch (adress)
